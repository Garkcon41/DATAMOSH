...
diff --git a//dev/null b/glitcher/effects.py
index 0000000000000000000000000000000000000000..9f4d027abaa26e9dafde4a1b30b158e8a724572c 100644
--- a//dev/null
+++ b/glitcher/effects.py
@@ -0,0 +1,234 @@
...
+
+    The implementation performs crude YCbCr conversion per block and heavily
+    quantises the luminance channel.  Chrominance channels are averaged per
+    block to emulate 4:2:0 subsampling.
+    """
+
+    if block_size <= 0:
...
+                # Quantise luma heavily.
+                quant_y = round(yv / luma_quant) * luma_quant
+                quant_y = max(0, min(255, quant_y))
+                # Blend chroma toward the average to emulate bleeding.
...
+                length = len(row)
+                shift = offset % length
+                out_row = row[shift:] + row[:shift]
+                out.pixels[y] = out_row
+
+    # Column shifts operate on transposed data to keep logic simple.
+    width = image.width
+    height = image.height
...
+    mode: str = "brightness",
+) -> Image:
+    """Sort pixels in runs where the brightness passes ``threshold``."""
+
+    if segment_length <= 0:
...
+                    end += 1
+                sorted_segment = sorted(row[x:end], key=lambda p: _pixel_metric(p, mode))
+                out.pixels[y][x:end] = sorted_segment
+                x = end
+            else:
+                x += 1
+
...
+    rng: random.Random | None = None,
+) -> List[Image]:
+    """Apply a crude datamosh-like smear across a sequence of frames."""
+
+    if not frames:
+        return []
+    if block_size <= 0:
...
+                    # Keep using the buffer block and smear it into the current frame.
+                    for y in range(by, min(by + block_size, frame.height)):
+                        for x in range(bx, min(bx + block_size, frame.width)):
+                            smear_pixel = buffer.pixels[y][x]
+                            working.pixels[y][x] = blend_pixels(
+                                working.pixels[y][x], smear_pixel, smear_strength
+                            )
+                else:
+                    # Update the buffer with the current frame's block (like inserting
+                    # a keyframe).
+                    for y in range(by, min(by + block_size, frame.height)):
+                        for x in range(bx, min(bx + block_size, frame.width)):
